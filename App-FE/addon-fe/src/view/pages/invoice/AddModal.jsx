...
const AddNewItem = ({ open, close }) => {
  ...
  useEffect(() => {
    sizeApi.get(initParms).then((res) => {
      setSize(res.data);
    });
    colorApi.get(initParms).then((res) => {
      setColor(res.data);
    });
    groupApi.get(initParms).then((res) => {
      setGroup(res.data);
    });
  }, [size,color,group]);
  let initForm = {
...
console.log(previewImage)
  const handleChange = ({ file: newFileList }) => setFileListDetail(newFileList);
      ...
      >
        ...
        >
          <Row gutter={[8, 0]}>
            ...
            <Col span={24}>
              <Form.Item name="upload" label={<Button onClick={()=> console.log('Da')}>Thêm ảnh</Button>}>
              <ImgCrop rotate>
                <Upload
                   accept="image/*"
